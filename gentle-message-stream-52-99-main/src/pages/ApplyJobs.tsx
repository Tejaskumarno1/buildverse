...
import { ArrowUpRight, Briefcase, Building, MapPin, Search, DollarSign, Calendar, Clock, Filter, Users, TrendingUp, Star, Globe, Award, Target, Zap, Brain, CheckCircle, XCircle, BarChart3 } from "lucide-react";
...
const ApplyJobs = () => {
  ...
  const performATSScanning = async (jobId: string, resumeData: any, job: any): Promise<ATSScoreData> => {
    try {
      console.log("Starting ATS scan for job:", jobId);
      console.log("Resume data:", resumeData);
      console.log("Job details:", job);
      
      // Create mock resume data exactly like atsScoreApi.ts does
        ...
        experience: [
          {
            ...
            startDate: "Jan 2020",
            endDate: "Present",
            description: `Developed and maintained web applications using React and Node.js. Led a team of 3 developers and improved system performance by 40%. Implemented automated testing that reduced bugs by 60%.`
          },
          {
            id: "2",
            ...
            startDate: "Jun 2019",
            endDate: "Dec 2019",
            description: `Built full-stack applications using modern JavaScript frameworks. Collaborated with cross-functional teams to deliver features on tight deadlines.`
          }
        ],
        skills: {
          professional: "Project Management, Business Analysis, Team Leadership",
          technical: job.skills_required?.slice(0, 5).join(", ") || "JavaScript, React, Node.js, TypeScript, Python",
          soft: "Communication, Teamwork, Problem Solving, Critical Thinking"
        },
        objective: `Experienced ${job.title || 'Software Engineer'} seeking to leverage technical expertise in ${job.department || 'technology'} to contribute to ${job.company_name || 'innovative company'}'s mission.`
      };

      console.log("Mock resume data created:", mockResumeData);
      console.log("Calling generateATSScore with mock data...");
      
      // Use the exact same generateATSScore function from atsScoreApi.ts
      const atsResult = await generateATSScore(mockResumeData);
      
      console.log("ATS scan completed successfully:", atsResult);
      console.log("Overall Score:", atsResult.overallScore);
      console.log("Keyword Score:", atsResult.keywordScore);
      console.log("Format Score:", atsResult.formatScore);
      console.log("Content Score:", atsResult.contentScore);
      console.log("Suggestions:", atsResult.suggestions);
      console.log("Job Match:", atsResult.jobMatch);
      
      // Store ATS results exactly like ATSScanner does
      setAtsResults(prev => ({
        ...prev,
        [jobId]: atsResult
      }));

      console.log("ATS results stored in state for job:", jobId);
      return atsResult;
      ...
      // Same error handling as atsScoreApi.ts
      toast({
  ...
  const handleApply = async (jobId: string) => {
    ...
    try {
      ...
      // Step 1: Perform ATS scanning with same logic as ATSScanner
      console.log("Step 1: Starting ATS scanning process...");
      toast({
      ...
      console.log("Step 2: ATS scanning completed, analyzing results...");
      
      // Step 2: Check if ATS score meets minimum threshold
      ...
      console.log(`- Candidate Score: ${candidateScore}/100`);
      console.log(`- Minimum Required: ${minimumScore}/100`);
      console.log(`- Passes ATS Check: ${passesATSCheck}`);

      // Step 3: Determine application status
      ...
      // Step 4: Ensure rating is within valid range (1-100)
      const validRating = Math.max(1, Math.min(100, Math.round(candidateScore)));
      console.log(`Valid Rating for DB: ${validRating}`);

      // Step 5: Submit application to database
      console.log("Step 5: Submitting application to database...");
      ...
      // Step 6: Store ATS scan results (same as ATSScanner)
      console.log("Step 6: Storing ATS scan results...");
        ...
        .insert([
          {
            ...
            overall_score: validRating,
            keyword_score: Math.max(1, Math.min(100, atsResult.keywordScore)),
            format_score: Math.max(1, Math.min(100, atsResult.formatScore)),
            content_score: Math.max(1, Math.min(100, atsResult.contentScore)),
      ...
      if (atsError) {
        ...
        console.log("ATS results stored successfully in database");
      }

      // Step 7: Notify HR members if application is approved
      if (passesATSCheck) {
        console.log("Step 7: Notifying HR members...");
        ...
        if (hrMembers && hrMembers.length > 0) {
          console.log(`Found ${hrMembers.length} HR members to notify`);
          ...
          console.log("HR notifications sent successfully");
        }
      }
      
      // Step 8: Show appropriate success/rejection message
      console.log("Step 8: Showing final result to user...");
      if (passesATSCheck) {
        toast({
        ...
        console.log("SUCCESS: Application approved and forwarded to HR");
      } else {
        toast({
        ...
        console.log("REJECTED: Application did not meet ATS requirements");
      }
      ...
      console.error("Unexpected error during application process:", err);
      ...
      console.log("Application process finished, processing state cleared");
    }
  };
                  ...
                  <Card key={job.id} className="group bg-white hover:bg-slate-50/50 border-0 shadow-lg hover:shadow-2xl transition-all duration-500 rounded-3xl overflow-hidden transform hover:-translate-y-1">
                    ...
                    <CardContent className="p-8">
                      <div className="space-y-8">
                        ...
                        {/* Enhanced Required Expertise Section */}
                          ...
                          <div>
                            <h4 className="font-bold text-lg text-slate-900 mb-4 flex items-center">
                              ...
                              <span className="ml-2 text-sm font-normal text-slate-600">
                                ({job.skills_required.length <= 2 ? 'Need 1+' : 
                                  job.skills_required.length === 3 ? 'Need 2+' :
                                  job.skills_required.length === 4 ? 'Need 3+' :
                                  job.skills_required.length === 5 ? 'Need 4+' :
                                  'Need 80%+'} matches)
                              </span>
                            </h4>
                          ...
                          <div className="bg-gradient-to-br from-cyan-50 to-blue-50 border-2 border-cyan-200 rounded-2xl p-6">
                            <div className="flex items-center gap-3 mb-4">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-slate-600">AI-powered resume analysis</p>
                              </div>
                            </div>
                    ...
                    <CardFooter className="bg-slate-50 border-t border-slate-100 px-8 py-6">
                      <div className="flex justify-between items-center w-full">
                        ...
                        <div className="flex space-x-4">
                          <Button 
                            variant="outline" 
                            className="border-slate-300 hover:bg-slate-100 px-6 py-3 rounded-xl font-semibold transition-all duration-300 hover:scale-[1.02]"
                          >
                            View Details
