...
serve(async (req) => {
  ...
  try {
    ...
    const subResourceId = pathSegments[4]; // 'application-id' or undefined
    ...
    if (resource === "jobs") {
      if (req.method === "GET") {
        if (id && subResource === "applications") {
          if (subResourceId && pathSegments[5] === "test-results") {
            // GET /api/jobs/:id/applications/:id/test-results
            const { data: testData, error: testError } = await supabase
              ...
              .select(`
                ...
                status,
                ai_score,
                typing_test_results!inner(*),
                interview_results!inner(*),
            ...
            if (testError) {
              if (testError.code === 'PGRST116') {
                // No test results found, try fetching application without test results
            ...
            return new Response(JSON.stringify({
              application: {
                ...
                ai_score: testData.ai_score,
                candidate: testData.candidates
              },
              typing_test_results: testData.typing_test_results[0],
              interview_results: testData.interview_results[0]
            }), {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 200,
            });
          } else if (subResourceId) {
            // GET /api/jobs/:id/applications/:id
        ...
        } else if (id && subResource === "test-config") {
          // GET /api/jobs/:id/test-config
          const { data, error } = await supabase
            .from("jobs")
            .select(`
              id,
        ...
        // Ensure ats_minimum_score has a default value if not provided
        if (!jobData.ats_minimum_score) {
          jobData.ats_minimum_score = 70;
        }

        // Validate ats_minimum_score is within valid range
        if (jobData.ats_minimum_score < 0 || jobData.ats_minimum_score > 100) {
          return new Response(JSON.stringify({ error: "ATS minimum score must be between 0 and 100" }), {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 400,
          });
        }
        ...
        // Validate ats_minimum_score if it's being updated
        if (jobUpdates.ats_minimum_score !== undefined) {
          if (jobUpdates.ats_minimum_score < 0 || jobUpdates.ats_minimum_score > 100) {
            return new Response(JSON.stringify({ error: "ATS minimum score must be between 0 and 100" }), {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 400,
            });
